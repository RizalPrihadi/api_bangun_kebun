...
namespace api_bangun_kebun.Contexts
{
    public class ContentContext
    {
        ...
        public bool createContent(Content content)
        {
            ...
            string query = @"insert into konten(pengguna_id_user, judul, gambar_video, isi_deskripsi, timestamp, jenis_konten_id_jenis_konten, like) 
                            values(@id_pengguna, @judul, @gambar_video, @isi_deskripsi, @timestamp, @id_jenis_konten, 0)";

            try
            {
                NpgsqlCommand cmd = db.GetNpgsqlCommand(query);
                cmd.Parameters.AddWithValue("@id_pengguna", content.id_user!);
                cmd.Parameters.AddWithValue("@judul", content.judul!);
